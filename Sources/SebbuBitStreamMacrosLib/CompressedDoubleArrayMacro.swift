...
public struct CompressedDoubleArrayMacro: PeerMacro {
    public static func expansion(of node: SwiftSyntax.AttributeSyntax, providingPeersOf declaration: some SwiftSyntax.DeclSyntaxProtocol, in context: some SwiftSyntaxMacros.MacroExpansionContext) throws -> [SwiftSyntax.DeclSyntax] {
        guard let variableDecl = declaration.as(VariableDeclSyntax.self), variableDecl.isStoredProperty else {
            throw DiagnosticsError(diagnostics: [
                BitStreamCodingDiagnostic.requiresStoredProperty("'@CompressedDoubleArray'").diagnose(at: Syntax(declaration))
            ])
        }
        try check(variable: variableDecl, attribute: "@CompressedDoubleArray")
        guard variableDecl.isArrayType(of: "Double") else {
            throw BitStreamCodingDiagnostic.custom("Double array compression can only be applied to Double arrays").error(at: Syntax(variableDecl))
        }
        return []
    }
    ...
    internal static func getSyntax(attribute: SwiftSyntax.AttributeSyntax, _ variableDecl: SwiftSyntax.VariableDeclSyntax) throws -> (String, String) {
        ...
        guard let intBits = Int(bits), intBits > 0, intBits <= 64 else {
            throw BitStreamCodingDiagnostic.custom("Parameter bits must be more than 0 and less than or equal to 64").error(at: Syntax(variableDecl))
        }
        
        guard let maxCountInt = Int(maxCount), maxCountInt > 0, maxCountInt <= 1 << 29 else {
            throw BitStreamCodingDiagnostic.custom("Parameter maxCount must be more than 0 and less than 2^29").error(at: Syntax(variableDecl))
        }
