...
public struct CompressedFloatArrayMacro: MemberAttributeMacro {
    ...
    internal static func getSyntax(attribute: SwiftSyntax.AttributeSyntax, _ variableDecl: SwiftSyntax.VariableDeclSyntax) throws -> (String, String) {
        ...
        guard variableDecl.variableType != nil else {
            //TODO: Provide fix-it
            let message = SimpleDiagnosticMessage(message: "Float array compression can only be applied to Float arrays. Explicitly provide the type annotation.", diagnosticID: MessageID(domain: "", id: "annotation"), severity: .error)
            let diagnostic = Diagnostic(node: Syntax(variableDecl), position: variableDecl.position, message: message)
            throw DiagnosticsError(diagnostics: [diagnostic])
        }
        guard variableDecl.isArrayType(of: "Float") else {
            //TODO: Provide fix-it
            let message = SimpleDiagnosticMessage(message: "Float array compression can only be applied to Float arrays", diagnosticID: MessageID(domain: "", id: "only-float-arrays"), severity: .error)
            let diagnostic = Diagnostic(node: Syntax(variableDecl), position: variableDecl.position, message: message)
            throw DiagnosticsError(diagnostics: [diagnostic])
        }
        ...
        guard let intBits = Int(bits), intBits > 0, intBits <= 32 else {
            let message = SimpleDiagnosticMessage(message: "bits must be more than 0 and less than or equal to 32", diagnosticID: MessageID(domain: "", id: "min < max"), severity: .error)
            let diagnostic = Diagnostic(node: Syntax(variableDecl), position: variableDecl.position, message: message)
            throw DiagnosticsError(diagnostics: [diagnostic])
        }
