...
extension Macro {
    ...
    static func arguments(from attribute: SwiftSyntax.AttributeSyntax, desiredArgumentCount: Int) throws -> [LabeledExprSyntax] {
        ...
        let arguments = argumentList.children(viewMode: .sourceAccurate).compactMap { $0.as(LabeledExprSyntax.self) }
    ...
    static func get(_ arguments: [LabeledExprSyntax], name: String, position: Int, as type: LiteralType...) throws -> String {
        if let value = arguments[position].expression.as(IntegerLiteralExprSyntax.self)?.literal.text, type.contains(.integer) {
            return value
        } else if let value = arguments[position].expression.as(FloatLiteralExprSyntax.self)?.literal.text, type.contains(.float) {
            return value
        } else if let operatorExpression = arguments[position].expression.as(PrefixOperatorExprSyntax.self) {
            let operatorToken = operatorExpression.operator.text
            if let value = operatorExpression.expression.as(FloatLiteralExprSyntax.self)?.literal.text, type.contains(.float) {
                return operatorToken + value
            } else if let value = operatorExpression.expression.as(IntegerLiteralExprSyntax.self)?.literal.text, type.contains(.integer) {
                return operatorToken + value
            }
        }
        switch type.first! {
